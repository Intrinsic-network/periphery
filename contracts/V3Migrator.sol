...
contract V3Migrator is IV3Migrator, PeripheryImmutableState, PoolInitializer, Multicall, SelfPermit {
    ...
    ) PeripheryImmutableState(_factory, _WRBTC) {
    ...
    function migrate(MigrateParams calldata params) external override {
        ...
        if (amount0V3 < amount0V2) {
            ...
            if (params.refundAsRBTC && params.token0 == WRBTC) {
                IWRBTC(WRBTC).withdraw(refund0);
                TransferHelper.safeTransferRBTC(msg.sender, refund0);
            } else {
        ...
        if (amount1V3 < amount1V2) {
            ...
            if (params.refundAsRBTC && params.token1 == WRBTC) {
                IWRBTC(WRBTC).withdraw(refund1);
                TransferHelper.safeTransferRBTC(msg.sender, refund1);
            } else {

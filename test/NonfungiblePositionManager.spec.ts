...
describe('NonfungiblePositionManager', () => {
  ...
  const nftFixture: Fixture<{
    ...
    const { wrbtc, factory, tokens, nft, router } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ;({ nft, factory, tokens, wrbtc, router } = await loadFixture(nftFixture))
  })
  ...
  describe('#createAndInitializePoolIfNecessary', () => {
    ...
    it('works if pool is created and initialized', async () => {
      ...
      await factory.createPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM)
      const pool = new ethers.Contract(expectedAddress, IIntrinsicPoolABI, wallet)
    ...
    it('could theoretically use rbtc via multicall', async () => {
      const [token0, token1] = sortedTokens(wrbtc, tokens[0])
  ...
  describe('#mint', () => {
    ...
    it('can use rbtc via multicall', async () => {
      const [token0, token1] = sortedTokens(wrbtc, tokens[0])

      // remove any approval
      await wrbtc.approve(nft.address, 0)
      ...
      const refundRBTCData = nft.interface.encodeFunctionData('refundRBTC')

      const balanceBefore = await wallet.getBalance()
      await nft.multicall([createAndInitializeData, mintData, refundRBTCData], {
    ...
    it('gas first mint for pool using rbtc with zero refund', async () => {
      const [token0, token1] = sortedTokens(wrbtc, tokens[0])
      await nft.createAndInitializePoolIfNecessary(
        token0.address,
        token1.address,
      ...
      await snapshotGasCost(
        nft.multicall(
          [
            ...
            nft.interface.encodeFunctionData('refundRBTC'),
          ],
          { value: 100 }
        )
      )
    ...
    it('gas first mint for pool using rbtc with non-zero refund', async () => {
      const [token0, token1] = sortedTokens(wrbtc, tokens[0])
      await nft.createAndInitializePoolIfNecessary(
        token0.address,
        token1.address,
  ...
  describe('#increaseLiquidity', () => {
    ...
    it('can be paid with RBTC', async () => {
      const [token0, token1] = sortedTokens(tokens[0], wrbtc)
      ...
      const refundRBTCData = nft.interface.encodeFunctionData('unwrapWRBTC', [0, other.address])
      await nft.multicall([mintData, refundRBTCData], { value: expandTo18Decimals(1) })
      ...
      await nft.multicall([increaseLiquidityData, refundRBTCData], { value: expandTo18Decimals(1) })
    })

...
describe('NonfungibleTokenPositionDescriptor', () => {
  const [wrbtc, ...wallets] = waffle.provider.getWallets()
  ...
  describe('#tokenRatioPriority', () => {
    it('returns -100 for WRBTC', async () => {
      expect(await nftPositionDescriptor.tokenRatioPriority(wrbtc.address, 1)).to.eq(-100)
    })
  ...
  describe('#flipRatio', () => {
    ...
    it('returns true if both tokens are denominators but token1 has lower priority ordering', async () => {
      expect(await nftPositionDescriptor.flipRatio(wrbtc.address, WBTC, 1)).to.eq(true)
    })
